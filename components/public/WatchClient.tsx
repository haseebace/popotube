...
export default function WatchClient({ tmdbId, movieDetails }: { tmdbId: string, movieDetails: any }) {
        ...
        async function checkStatus() {
            ...
            try {
                ...
                if (!checkData.exists) {
                     ...
                     if (vid.status === 'completed' && (vid.playback_source?.url || vid.stream_url)) {
    ...
    // Native proxy intercept to bypass Real-Debrid forced-download headers
    let finalPlaybackUrl = status?.playback_source?.url || status?.stream_url;
    if (status?.playback_source?.type === 'direct' && status?.id) {
        finalPlaybackUrl = `/api/proxy/stream/${status.id}`;
    }
    
    const isProxyType = status?.playback_source?.type === 'proxy' || status?.playback_source?.type === 'mediaflow';
                     ...
                     {status?.playback_source?.is_streamable === false && !isProxyType ? (
                         <div className="text-center p-8 space-y-4">
                             <p className="text-muted-foreground">
                               This file container ({status.playback_source.container}) cannot be played directly in the browser yet.
                             </p>
                         ...
                         >
                             ...
                             {status?.playback_source?.captions?.map((cap: any, idx: number) => (
                                 <track 
                                     key={idx} 
                                     kind="subtitles" 
                                     src={cap.url} 
