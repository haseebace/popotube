...
function StatusIcon({ status }: { status: string }) {
  if (status === "active")
    return (
      <PlayCircle className="w-4 h-4 text-blue-500 animate-pulse shrink-0" />
    );
  if (status === "completed")
...
export default function AdminPage() {
  ...
  const fetchStats = useCallback(async (isManual = false) => {
    ...
    try {
      const res = await fetch(publicBackendApiUrl("/api/dashboard/stats"));
  ...
  const daysLeft = stats?.rdUser?.expiration
    ? Math.ceil(
        (new Date(stats.rdUser.expiration).getTime() - Date.now()) /
          (1000 * 60 * 60 * 24),
      )
    : null;
    ...
    <div className="w-full max-w-6xl mx-auto px-4 md:px-8 py-10 space-y-10">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Queue activity, library count, and Real-Debrid connection status.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {error && (
        <div className="flex items-center space-x-3 px-4 py-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 text-sm">
          ...
          <span>
            Couldn&apos;t load dashboard stats ({error}). Check that the backend
            is running, then try Refresh.
          </span>
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge
            variant="outline"
            className="bg-blue-500/10 text-blue-500 border-none gap-1"
          >
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm">
                        Nothing in the queue. Start an ingest from Search to see
                        jobs here.
                      </span>
                    </div>
                  </TableCell>
                  ...
                  <TableRow key={job.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className="font-medium text-sm truncate max-w-[380px]"
                          title={job.name}
                        >
                          {job.name}
                        </span>
