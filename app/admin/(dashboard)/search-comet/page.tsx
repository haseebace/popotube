...
export default function CometSearchPage() {
  const router = useRouter();
  const [selectedMovie, setSelectedMovie] = useState<SelectedMovie | null>(
    null,
  );
  const [results, setResults] = useState<CometResult[]>([]);
  ...
  async function handleIngest(result: CometResult) {
    ...
    <div className="w-full max-w-[1400px] mx-auto px-4 md:px-8 py-10">
      <div className="mb-10 text-center">
        ...
        <p className="text-muted-foreground font-medium">
          Torrent results from the Comet indexer
        </p>
      </div>
      ...
      <div className="mb-10 bg-card border rounded-xl p-6 max-w-2xl mx-auto flex flex-col gap-3">
        ...
        {selectedMovie && (
          <Badge
            variant="secondary"
            className="w-fit bg-primary/5 text-primary border-primary/20 py-1.5 px-3 flex items-center gap-2"
          >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                if (selectedMovie?.id != null) fetchStreams(selectedMovie.id);
              }}
              className="h-8"
            >
              <RefreshCw
        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow>
              ...
              <TableHead className="w-[180px] pr-6 text-right">
                Action
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                ...
                >
                  {selectedMovie
                    ? "No streams for this title. Try another release or source."
                    : "Pick a TMDb title to load Comet results."}
                </TableCell>
              </TableRow>
            ) : (
                    ...
                    <TableCell className="pl-6 max-w-[400px]">
                      <div className="flex flex-col gap-1 py-1">
                        ...
                        <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] uppercase">
                          {seLabel && (
                            <Badge variant="outline">{seLabel}</Badge>
                          )}
                    ...
                    <TableCell className="pr-6 text-right">
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (res.magnetUri)
                              navigator.clipboard.writeText(res.magnetUri);
