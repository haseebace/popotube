    ...
    async function fetchRDUser() {
      try {
        const res = await fetch(
          publicBackendApiUrl("/api/settings/real-debrid/user"),
        );
        if (res.ok) {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Real-Debrid profile used by this backend.
          </p>
        </div>
        ...
        <CardContent className="space-y-6">
          {loading ? (
            ...
            <div className="text-red-400 text-sm">
              Couldn&apos;t load Real-Debrid account. Set REAL_DEBRID_API_KEY in
              the environment or under Integrations, then refresh.
            </div>
          )}
        </CardContent>
      </Card>
