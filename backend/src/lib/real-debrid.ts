...
/** Safe for logs — never includes Authorization or full axios config. */
function rdRequestLogFields(error: unknown): Record<string, unknown> {
  const e = error as {
    message?: string;
  ...
  const retryAfter =
    h && typeof h["retry-after"] === "string"
      ? h["retry-after"]
      : h && typeof h["Retry-After"] === "string"
        ? h["Retry-After"]
        : undefined;
  let rdBody: unknown = e.response?.data;
